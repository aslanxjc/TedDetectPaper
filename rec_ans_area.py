    ...
    def get_ans_path(self,std_point=None):
        ...
        if len(cnts) > 0:
            ...
            for c in cnts:
                ...
                if w>1000:
                    print x,y,w,h
                #if w > self.width/5 and x>100 and y>100 and h>100 and h<250:
                if w > self.width/5 and x>100 and y>100 and h>100 and h<500:
            ...
            cut_point_lst = sorted(cut_point_lst,key=lambda x:x["w"],reverse=True)
            for _cp in cut_point_lst:
                x = _cp.get("x")
                y = _cp.get("y")
                w = _cp.get("w")
                h = _cp.get("h")
                _cut_point = (x+5,y+5,x+w-10,y+h-10)
                _cut_path = self.cut_ans_area(_cut_point)
                #计算该区域下的填图轮廓数
                from rec_cnts import RecCnts
                reccnts_obj = RecCnts(cut_path)
                rec_cnts = reccnts_obj.get_cnts()
